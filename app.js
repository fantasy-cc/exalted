...
async function loadExchangeRates(force = false) {
    if (exchangeRatesPromise) {
        ...
        try {
            ...
            console.warn('⚠️ Previous exchange rate fetch encountered an error:', error.message);
        }
    }

    const league = encodeURIComponent(currentConfig.league);
    const url = `${API_BASE_URL}/rates/${league}?top_percentage=${currentConfig.topPercentage}`;
...
async function ensureExchangeRatesLoaded({ forceRefresh = false } = {}) {
    const hasRates = currentRates && currentRates.rates && Object.keys(currentRates.rates).length > 0;

    if (!forceRefresh && hasRates) {
        return currentRates;
    }

    try {
        const data = await loadExchangeRates(forceRefresh);

        if (!data || !data.rates || Object.keys(data.rates).length === 0) {
            throw new Error('Exchange rate response did not include rate data');
        }

        return data;
    } catch (error) {
        console.error('❌ Unable to prepare exchange rates for budget calculations:', error);
        throw error;
    }
}
...
async function initializeApp() {
    ...
    // Load currencies and then ensure exchange rates are available for budget calculations
    await loadCurrencies();

    try {
        await ensureExchangeRatesLoaded();
    } catch (error) {
        console.warn('⚠️ Initial exchange rate load failed:', error);
        updateStatus(`⚠️ Live exchange rates unavailable (${error.message}). Using fallback values until refreshed.`, 'warning');
    }
    
    // Set up event listeners
    setupEventListeners();
    
    console.log(`✅ App initialized successfully${currentRates && currentRates.rates ? '' : ' (awaiting live exchange rates)'}`);
}
...
async function handleTopPercentageChange(event) {
    ...
    try {
        await ensureExchangeRatesLoaded({ forceRefresh: true });
        updateStatus(`✅ Exchange rates updated for top ${percentage}% of currencies`, 'success');
    } catch (error) {
        console.warn('⚠️ Failed to refresh exchange rates after top percentage change:', error);
        updateStatus(`⚠️ Currencies updated, but exchange rate refresh failed: ${error.message}`, 'warning');
    }
}
...
async function forceRefreshData() {
    ...
    try {
        ...
        const refreshUrl = `${API_BASE_URL}/arbitrage/${encodeURIComponent(DEFAULT_LEAGUE)}?starting_currency=chaos&amount=100&min_profit=0.01&max_results=1&top_percentage=${currentConfig.topPercentage}`;
        ...
        try {
            ...
            console.warn('⚠️ Unable to refresh exchange rates after live data refresh:', rateError);
        }

        if (ratesUpdated) {
            updateStatus('✅ Live data refresh completed! Exchange rates updated for budget calculations.', 'success');
        } else {
            updateStatus('⚠️ Live data refreshed, but exchange rates failed to update. Using fallback values until next refresh.', 'warning');
        }
...
async function calculateArbitrage() {
    ...
    try {
        try {
            ...
            console.warn('⚠️ Could not refresh live exchange rates before calculation:', rateError);
        }
